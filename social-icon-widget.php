...
__('Social Icon Widget', 'social_icon_domain'),

// Widget description
array( 'description' => __('Social Icon Buttons Widget', 'social_icon_domain'), )
);
    }
...
echo '<div class="social-icons" style="max-width:100%;">';
    ...
    if (! empty($facebook)) {
        echo '<a href="https://facebook.com/' . $facebook . '" class="facebook" target="_blank" title="Facebook"><i class="social fa fa-facebook" aria-hidden="true"></i><span>Facebook Page</span></a>';
    }

    $twitter = $instance['twitter'];
    if (! empty($twitter)) {
        echo '<a href="https://twitter.com/' . $twitter . '" class="twitter" target="_blank" title="Twitter"><i class="social fa fa-twitter" aria-hidden="true"></i><span>Twitter Profile</span></a>';
    }

    $googleplus = $instance['googleplus'];
    if (! empty($googleplus)) {
        echo '<a href="https://plus.google.com/' . $googleplus . '" class="google-plus" target="_blank" title="Google+"><i class="social fa fa-google-plus" aria-hidden="true"></i><span>Google+ Profile</span></a>';
    }

    $pinterest = $instance['pinterest'];
    if (! empty($pinterest)) {
        echo '<a href="https://pinterest.com/' . $pinterest . '" class="pinterest" target="_blank" title="Pinterest"><i class="social fa fa-pinterest-p" aria-hidden="true"></i><span>Pinterest Profile</span></a>';
    }

    $feedly = $instance['feedly'];
    if (! empty($googleplus)) {
        echo '<a href="' . $feedly . '" class="feedly" target="_blank" title="Feedly"><i class="social fa fa-rss" aria-hidden="true"></i><span>Feedly RSS Feed</span></a>';
    }

    $email = $instance['email'];
    if (! empty($email)) {
        echo '<a href="mailto:' . $email . '" class="email" target="_blank" title="Email"><i class="social fa fa-envelope" aria-hidden="true"></i><span>Send email</span></a>';
    }

    $print = $instance['print'];
    if (! empty($print)) {
        echo '<a href="#" onclick="window.print()" class="print" target="_blank"><i class="social fa fa-print" aria-hidden="true" title="Print"></i><span>Print page</span></a>';
    }
...
public function form($instance)
{
    if (isset($instance[ 'title' ])) {
        ...
        $title = __('New title', 'social_icon_domain');
    }
...
?>
	...
	<p>
		<label for="<?php echo $this->get_field_id('feedly'); ?>"><?php _e('Feedly:'); ?></label>
	...
	<p>
		<label for="<?php echo $this->get_field_id('print'); ?>"><?php _e('Print:'); ?></label>
